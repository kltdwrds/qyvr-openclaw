...
function schnorrGetExtPubKey(priv) {
  ...
  const d_ = Fn.fromBytes(abytes3(priv, 32, "secretKey"));
...
function schnorrSign(message, secretKey2, auxRand = randomBytes2(32)) {
  const { Fn, BASE } = Pointk1;
...
function schnorrGetPublicKey2(secretKey2) {
  return schnorrGetExtPubKey2(secretKey2).bytes;
}
function schnorrSign2(message, secretKey2, auxRand = randomBytes(32)) {
  const { Fn } = Pointk12;
  const m = abytes(message, void 0, "message");
  const { bytes: px, scalar: d } = schnorrGetExtPubKey2(secretKey2);
...
var JS = class {
  ...
  getPublicKey(secretKey2) {
    return bytesToHex(schnorr2.getPublicKey(secretKey2));
  }
  finalizeEvent(t, secretKey2) {
    const event = t;
    event.pubkey = bytesToHex(schnorr2.getPublicKey(secretKey2));
    event.id = getEventHash(event);
    event.sig = bytesToHex(schnorr2.sign(hexToBytes(getEventHash(event)), secretKey2));
  ...
  verifyEvent(event) {
    ...
    try {
      ...
      const valid = schnorr2.verify(hexToBytes(event.sig), hexToBytes(hash), hexToBytes(event.pubkey));
...
function pubkeyOf(skHex) {
  ...
  return bytesToHex2(schnorr.getPublicKey(hexToBytes2(skHex)));
}
...
var payloadHash = (bodyText) => bytesToHex2(sha256(utf8ToBytes(bodyText)));
function signNip98(skHex, method, url, bodyText) {
  ...
  const ev = finalizeEvent({ kind: NIP98_KIND, created_at: Math.floor(Date.now() / 1e3), content: "", tags }, hexToBytes2(skHex));
...
var RelayHttp = class {
  ...
  async submit(template, { includeAuthTag = false } = {}) {
    ...
    const ev = finalizeEvent({ created_at: Math.floor(Date.now() / 1e3), ...template, tags }, hexToBytes2(this.sk));
...
// packages/cli/src/main.mjs
var USAGE = `usage: qyvr <command> [options]

  enroll --name <name> [--harness <h>] [--model <m>]   ask the owner to approve this key
  attest                                               fetch and cache a fresh attestation; name this key in Buzz
  env                                                  print: export BUZZ_AUTH_TAG='...'
  ask --reason <text> --uses <action=n,...> [--plan <file>] [--room <id>] [--expires-in <s>] [--budget <tokens>]
  do <open-room|add-to-room|admit-guest|hire|provision|release> --grant <id> [--<arg> <value> ...]
       provision --candidate <index slug> --name <name> --room <id>;  release --agent <npub|name>
  grants                                               list this agent's live grants
  candidates [--query <text>] [--limit <n>]            search deployable Agent Index listings
  hires                                                list the hires this agent provisioned
  post --type <brief|artifact|decision|claim|blocker> --summary <line> [--payload-file <json> | --text-file <md>] [--room <id>]
                                                       post a typed message (default room: #homeroom)

environment: BUZZ_PRIVATE_KEY (hex or nsec, required), BUZZ_AUTH_TAG, BUZZ_RELAY_URL,
             QYVR_CONTROL_URL (default https://buzz.qyvr.ai), QYVR_HOME (default ~/.qyvr)`;
...
var badInput = (message) => new CliError(1, "bad_input", message);
function parseArgs(argv) {
  ...
  for (let i2 = 0; i2 < argv.length; i2++) {
    ...
    if (a.startsWith("--")) {
      const key = a.slice(2);
      const v = argv[i2 + 1];
      if (v === void 0 || v.startsWith("--")) throw badInput(`--${key} needs a value`);
      out[key] = v;
      i2++;
    } else out._.push(a);
  }
...
function secretKey(env) {
  const raw = (env.BUZZ_PRIVATE_KEY ?? "").trim();
  if (!raw) throw badInput("set BUZZ_PRIVATE_KEY to this agent's secret key (hex or nsec)");
  if (/^[0-9a-f]{64}$/i.test(raw)) return raw.toLowerCase();
  if (raw.startsWith("nsec1")) {
    const { type, data } = decode(raw);
    if (type === "nsec") return Buffer.from(data).toString("hex");
  }
  throw badInput("BUZZ_PRIVATE_KEY must be 64 hex characters or an nsec");
}
async function main(argv, env, io) {
  try {
    const args = parseArgs(argv);
    const [cmd, sub] = args._;
    const commands = { enroll, attest, env: printEnv, ask, do: doAction, grants, candidates, hires, post };
    if (!commands[cmd]) throw badInput(USAGE);
    const ctx = makeContext(env);
    const out = await commands[cmd](ctx, args, sub, io);
    if (out !== void 0) io.stdout(typeof out === "string" ? `${out}
` : `${JSON.stringify(out, null, 2)}
`);
    return 0;
  } catch (e) {
    if (e instanceof CliError) {
      io.stderr(e.exit === 1 && e.code === "bad_input" ? `${e.message}
` : `${e.code}: ${e.message}
`);
      return e.exit;
    }
    io.stderr(`error: ${e?.message ?? e}
`);
    return 4;
  }
}
function makeContext(env) {
  const control = (env.QYVR_CONTROL_URL || "https://buzz.qyvr.ai").replace(/\/+$/, "");
  const home = env.QYVR_HOME || join2(homedir(), ".qyvr");
  let sk = null;
  const ctx = {
    env,
    control,
    ...
    pk() {
      return pubkeyOf(ctx.sk());
    },
    dir() {
      const d = join2(home, ctx.pk());
      mkdirSync(d, { recursive: true, mode: 448 });
      return d;
    },
    async api(method, path, body) {
      const url = `${control}${path}`;
      const text = body === void 0 ? "" : JSON.stringify(body);
      const authorization = signNip98(ctx.sk(), method, url, text || void 0);
      let res;
      try {
        res = await fetch(url, { method, headers: { Authorization: authorization, "Content-Type": "application/json" }, body: text || void 0 });
      } catch (e) {
        throw new CliError(2, "network", `cannot reach ${control}: ${e?.cause?.code ?? e.message}`);
      }
      ...
      if (!res.ok) throw new CliError(3, json?.error ?? `http_${res.status}`, json?.message ?? raw);
      return json;
    },
    async info() {
      let res;
      try {
        res = await fetch(`${control}/v1/info`);
      } catch (e) {
        throw new CliError(2, "network", `cannot reach ${control}: ${e?.cause?.code ?? e.message}`);
      }
      if (!res.ok) throw new CliError(3, `http_${res.status}`, "the control plane did not answer /v1/info");
      return res.json();
    },
    authTag() {
      if (env.BUZZ_AUTH_TAG) {
        try {
          return JSON.parse(env.BUZZ_AUTH_TAG);
        } catch {
          throw badInput("BUZZ_AUTH_TAG is not valid JSON");
        }
      }
      try {
        return JSON.parse(readFileSync(join2(home, ctx.pk(), "auth-tag"), "utf8"));
      } catch {
        throw badInput("no attestation yet: run `qyvr attest` first");
      }
    }
  };
  return ctx;
}
async function enroll(ctx, args, _sub, io) {
  if (!args.name) throw badInput("enroll needs --name <name>");
  const body = { name: args.name };
  if (args.harness) body.harness = args.harness;
  if (args.model) body.model = args.model;
  const r = await ctx.api("POST", "/v1/enroll", body);
  io.stderr(`Open this link while signed in as the owner to approve ${args.name}, then run \`qyvr attest\`:
${r.url}
`);
  return r;
}
async function attest(ctx) {
  const r = await ctx.api("POST", "/v1/attest", {});
  const file = join2(ctx.dir(), "auth-tag");
  writeFileSync(file, JSON.stringify(r.tag), { mode: 384 });
  chmodSync(file, 384);
  const out = { expires_at: r.expires_at, cached: file };
  if (r.profile) out.profile = await publishProfile(ctx, r.tag, r.profile);
  return out;
}
async function publishProfile(ctx, tag, profile) {
  try {
    const relayUrl = ctx.env.BUZZ_RELAY_URL || (await ctx.info()).relay_url;
    return await ensureProfile(new RelayHttp(relayUrl, ctx.sk(), { authTag: tag }), profile, { keepName: true });
  } catch (e) {
    return `failed: ${e.message}`;
  }
}
function printEnv(ctx) {
  const tag = ctx.authTag();
  return `export BUZZ_AUTH_TAG='${JSON.stringify(tag).replace(/'/g, "'\\''")}'`;
}
function parseUses(text) {
  const uses = {};
  for (const part of String(text ?? "").split(",").map((s) => s.trim()).filter(Boolean)) {
    const m = /^([a-z-]+)=(\d+)$/.exec(part);
    if (!m) throw badInput(`--uses must look like open-room=2,hire=1 (got "${part}")`);
    uses[m[1]] = Number(m[2]);
  }
  if (Object.keys(uses).length === 0) throw badInput("ask needs --uses <action=n,...>");
  return uses;
}
function nextNumber(ctx, counter) {
  const file = join2(ctx.dir(), counter);
  let n = 0;
  try {
    n = Number(readFileSync(file, "utf8")) || 0;
  } catch {
  }
  writeFileSync(file, String(n + 1), { mode: 384 });
  return n + 1;
}
async function ask(ctx, args) {
  if (!args.reason) throw badInput("ask needs --reason <text>");
  const payload = { reason: args.reason, uses: parseUses(args.uses) };
  if (args.plan) {
    try {
      payload.plan = readFileSync(args.plan, "utf8");
    } catch {
      throw badInput(`cannot read --plan ${args.plan}`);
    }
  }
  if (args.budget !== void 0) {
    if (!/^\d+$/.test(args.budget)) throw badInput("--budget must be a whole number of tokens");
    payload.budget_tokens = Number(args.budget);
  }
  if (args["expires-in"] !== void 0) {
    if (!/^\d+$/.test(args["expires-in"])) throw badInput("--expires-in must be a whole number of seconds");
    payload.expires_in_s = Number(args["expires-in"]);
  }
  const { ev, room } = await submitTyped(ctx, args.room, "ask", "ask-counter", args.reason.split("\n")[0].slice(0, 120), payload);
  const ping = await ctx.api("POST", "/v1/asks/ping", {});
  return { ask: ev.id, room, new_asks: ping.new_asks };
}
async function submitTyped(ctx, roomArg, type, counter, summary, payload) {
  const tag = ctx.authTag();
  const info = roomArg && ctx.env.BUZZ_RELAY_URL ? null : await ctx.info();
  const room = roomArg ?? info.homeroom;
  const relayUrl = ctx.env.BUZZ_RELAY_URL || info.relay_url;
  const relay = new RelayHttp(relayUrl, ctx.sk(), { authTag: tag });
  try {
    const ev = await relay.submit(typedMessage({ channelId: room, type, n: nextNumber(ctx, counter), summary, payload }), { includeAuthTag: true });
    return { ev, room };
  } catch (e) {
    throw new CliError(e.status ? 3 : 2, "relay_error", e.message);
  }
}
async function doAction(ctx, args, action) {
  if (!action) throw badInput("do needs an action: open-room, add-to-room, admit-guest, hire, provision or release");
  if (!args.grant) throw badInput("do needs --grant <id> (see `qyvr grants`)");
  const body = {};
  for (const [k, v] of Object.entries(args)) {
    if (k === "_") continue;
    const key = k.replace(/-/g, "_");
    body[key] = key === "ttl_s" && /^\d+$/.test(v) ? Number(v) : v;
  }
  return ctx.api("POST", `/v1/do/${action}`, body);
}
async function grants(ctx) {
  return ctx.api("GET", "/v1/grants");
}
async function candidates(ctx, args) {
  const q = new URLSearchParams();
  if (args.query) q.set("q", args.query);
  if (args.limit !== void 0) {
    if (!/^\d+$/.test(args.limit)) throw badInput("--limit must be a whole number");
    q.set("limit", args.limit);
  }
  const qs = q.toString();
  return ctx.api("GET", `/v1/candidates${qs ? `?${qs}` : ""}`);
}
async function hires(ctx) {
  return ctx.api("GET", "/v1/hires");
}
var POST_TYPES = ["brief", "artifact", "decision", "claim", "blocker"];
async function post(ctx, args) {
  if (!POST_TYPES.includes(args.type)) {
    const why = args.type === "ask" ? " (use `qyvr ask`)" : ["grant", "receipt"].includes(args.type) ? " (only the control plane posts those)" : "";
    throw badInput(`post needs --type ${POST_TYPES.join("|")}${why}`);
  }
  if (!args.summary || /[\r\n]/.test(args.summary)) throw badInput("post needs --summary <one line>");
  if (args["payload-file"] && args["text-file"]) throw badInput("give --payload-file or --text-file, not both");
  let payload = {};
  if (args["payload-file"]) {
    let raw;
    try {
      raw = readFileSync(args["payload-file"], "utf8");
    } catch {
      throw badInput(`cannot read --payload-file ${args["payload-file"]}`);
    }
    try {
      payload = JSON.parse(raw);
    } catch {
      throw badInput(`--payload-file ${args["payload-file"]} is not valid JSON`);
    }
  } else if (args["text-file"]) {
    try {
      payload = { text: readFileSync(args["text-file"], "utf8") };
    } catch {
      throw badInput(`cannot read --text-file ${args["text-file"]}`);
    }
  }
  const { ev, room } = await submitTyped(ctx, args.room, args.type, "post-counter", args.summary.slice(0, 200), payload);
  return { event: ev.id, type: args.type, room };
}

// packages/cli/bin/qyvr.mjs
