...
var USAGE = `usage: qyvr <command> [options]
  ...
  do <open-room|add-to-room|admit-guest|hire> --grant <id> [--<arg> <value> ...]
...
async function main(argv, env, io) {
  try {
    ...
    const commands = { enroll, attest, env: printEnv, ask, do: doAction, grants };
...
function nextAskNumber(ctx) {
  const file = join2(ctx.dir(), "ask-counter");
  let n = 0;
  try {
...
async function ask(ctx, args) {
  ...
  const tag = ctx.authTag();
  const info = args.room && ctx.env.BUZZ_RELAY_URL ? null : await ctx.info();
  const room = args.room ?? info.homeroom;
  const relayUrl = ctx.env.BUZZ_RELAY_URL || info.relay_url;
  const relay = new RelayHttp(relayUrl, ctx.sk(), { authTag: tag });
  let ev;
  try {
    ev = await relay.submit(typedMessage({ channelId: room, type: "ask", n: nextAskNumber(ctx), summary: args.reason.split("\n")[0].slice(0, 120), payload }), { includeAuthTag: true });
  } catch (e) {
...
async function doAction(ctx, args, action) {
  if (!action) throw badInput("do needs an action: open-room, add-to-room, admit-guest or hire");
