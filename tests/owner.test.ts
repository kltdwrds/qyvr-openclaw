...
for (const kind of ["group", "direct", "email"]) for (const role of ["owner", "member"]) test(`roster owner identity preserves routing: ${kind}, ${role}`, async t => {
  ...
  const chat = { uid: "chat", status: "active", trusted: false, participants: [sender, agent, ...(kind === "group" ? [{ ...sender, uid: "other", role: "member", display_name: "Other" }] : [])] };
  ...
  server.on("connection", (socket: { send: (text: string) => void }) => socket.send(JSON.stringify({ event_type: "message_received", event_id: "event", chat_id: "chat", data: { message: { uid: "inbound", direction: "inbound", sender: { ...sender, role: "owner" }, body: "hello", attachments: [], created_at: new Date().toISOString() } } })));
  type Peer = { kind: string; id: string };
  let routingPeer: Peer | undefined;
  let context: { from: string; sender: { id: string }; conversation: { id: string; routePeer: Peer }; message: { bodyForAgent: string } } | undefined;
  ...
  entry.register({ registrationMode: "full", registerTool() {}, logger: { info() {} }, on() {},
    ...
    runtime: { channel: {
      ...
      inbound: { buildContext: async (value: typeof context) => { context = value; return {}; }, dispatch: async () => {
